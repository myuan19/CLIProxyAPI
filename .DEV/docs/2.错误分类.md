...
| 模型不存在 | 404 | 不同节点可能配了不同模型，该节点没有但其他可能有 |
...
| 无法处理 | 422 | 请求体格式合法但语义有误 |

### 3. 灰色地带（需要特殊判断）

400 和 404 不是非黑即白，需要检查错误信息做进一步分类：

| 错误类型 | 当前处理 | 说明 |
|---------|---------|------|
| 400 + overload 关键词 | **重试** | 某些 provider 用 400 返回过载信息（`isOverloadMessage` 检测） |
| 400 + token/auth 关键词 | **重试** | 某些 provider 用 400 返回凭证问题（`isTokenError` 检测），换凭证可能有效 |
| 400 (其他) | 不重试 | 普通的请求格式错误 |
| 404 模型不存在 | **重试** | 不同节点可能配了不同模型，该节点没有但其他有。会触发冷却 |
| 网络层错误 | **重试** | 如果所有节点共用同一个 proxy/出口，则全部不通，但无法预判 |

#### `isTokenError` 检测的关键词

400 状态码的错误信息中如果包含以下关键词，则判定为 token/凭证问题，标记为 Retryable：
...
"invalid bearer token", "expired token", "expired key",
"invalid credential", "invalid authorization"
```

这解决了某些 provider（如部分第三方中转）返回 400 而非 401 来表示 token 无效的问题。
...
流式请求（`ExecuteStreamWithFailover`）的 `executeFunc` 需要和上游建立连接并返回一个 `chunks` channel。
如果上游响应极慢（如 TCP 握手卡住、TLS 协商超时等），`executeFunc` 会一直阻塞，直到**父请求的 context** 超时（通常 60s）。

此时产生的错误是 `context.Canceled`，被 `ClassifyError` 正确分类为 **NonRetryable**（客户端取消）——但这里的"取消"并非用户主动发起，而是父 context 到期。结果是：

- **不触发冷却** — 故障节点不会被隔离
- **不尝试下一个节点** — 直接返回错误给用户
- **failoverFirstChunkTimeout（10s）根本没有启动** — 它原本在 `executeFunc` 返回之后才开始计时

### 修复方案

将 `executeFunc` 放入独立 goroutine，同时启动 `failoverFirstChunkTimeout` 计时器，用 `select` 同时等待连接结果和超时：
...
```
    ...
    Phase 1: 连接建立
         │
    ├── 超时 → "connection timeout" (Retryable) → 冷却 + 下一个目标
    │         同时后台 drain orphaned channel 防止 goroutine 泄漏
    │
    └── 成功 → Phase 2: 复用同一个 timer 等首个 chunk
                │
                ├── 超时 → "first chunk timeout" (Retryable) → 冷却 + 下一个目标
                │         同时后台 drain channel 防止 goroutine 泄漏
                │
                └── 成功 → 正常转发流式数据
```

### context.Canceled 的分类说明

| 场景 | 错误 | 分类 | 是否冷却 |
|------|------|------|---------|
| 用户主动取消请求 | `context.Canceled` | NonRetryable | ❌ 不冷却（用户行为） |
| 上游连接慢，10s timer 触发 | `connection timeout` | Retryable | ✅ 冷却（节点问题） |
| 连接成功但首个 chunk 慢，timer 触发 | `first chunk timeout` | Retryable | ✅ 冷却（节点问题） |
| ~~上游慢，父 ctx 60s 超时~~（已修复） | ~~`context.Canceled`~~ | ~~NonRetryable~~ | ~~❌ 不冷却~~ |

修复后，最后一行场景不会再发生——10s timer 会在父 context 超时之前介入。

---

## 总结

> 当前实现中，重试和冷却是绑定的：`Retryable` → 重试下一个节点 + 当前节点进入冷却 + 调度健康检查；`NonRetryable` → 立即返回错误，不重试、不冷却、不健康检查。

| 错误场景 | 分类 | 重试 + 冷却 |
|---------|------|-----------|
| 401 Unauthorized | Retryable | ✅ 重试下一个节点，当前节点冷却 |
| 402 Payment Required | Retryable | ✅ |
| 403 Forbidden | Retryable | ✅ |
| 404 Not Found | Retryable | ✅ 不同节点可能配了不同模型 |
| 429 Rate Limited | Retryable | ✅ |
| 5xx Server Error | Retryable | ✅ |
| 400 + overload 关键词 | Retryable | ✅ provider 用 400 表示过载 |
| 400 + token/auth 关键词 | Retryable | ✅ provider 用 400 表示凭证无效 |
| 400（普通） | NonRetryable | ❌ 直接返回错误 |
| 413 Payload Too Large | NonRetryable | ❌ |
| 422 Unprocessable | NonRetryable | ❌ |
| 网络错误（连接超时/拒绝/重置） | Retryable | ✅ |
| 流式连接/首 chunk 超时（10s） | Retryable | ✅ |
| `context.Canceled`（用户取消） | NonRetryable | ❌ |
| 无法判断（默认） | Retryable | ✅ 保守策略，宁可多试 |