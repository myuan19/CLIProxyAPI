...
#    - 位置: CLIProxyAPI/_scripts/.tmp/config.test.yaml
#    - 清理: 脚本结束时自动删除
#
# 2. 创建临时目录
#    - 影响: 创建 auths-test 和 logs-test 目录
#    - 位置: CLIProxyAPI/auths-test, CLIProxyAPI/_scripts/.tmp/logs-test
#    - 清理: 脚本结束时自动删除
#
# 3. 构建 Docker 镜像
...
# 使用说明
# 保存脚本：位于 CLIProxyAPI/_scripts/docker-run-and-release.sh
...
# 钩子脚本目录（项目内的 hook-scripts/ 会被挂载到运行时数据目录）
# 容器内路径: {auth-dir}/unified-routing/hook-scripts
HOOK_SCRIPTS_DIR_NAME="hook-scripts"
...
PROJECT_ROOT="$(cd "$SCRIPT_PATH/.." && pwd)"  # 从 _scripts/ 往上一层到项目根目录
...
TEST_CONFIG_FILE="$PROJECT_ROOT/_scripts/.tmp/config.test.yaml"
TEST_AUTH_DIR="$PROJECT_ROOT/auths-test"
TEST_LOG_DIR="$PROJECT_ROOT/_scripts/.tmp/logs-test"
LOCAL_DOCKERFILE="$PROJECT_ROOT/_scripts/.tmp/Dockerfile.local"
BACKEND_STATIC_DIR="$PROJECT_ROOT/_scripts/.tmp/static"
BACKEND_MANAGEMENT_HTML="$BACKEND_STATIC_DIR/management.html"

# 系统目录路径（使用顶部配置的变量）
SYSTEM_AUTH_DIR="$SYSTEM_AUTH_DIR_PATH"
SYSTEM_LOG_DIR="$PROJECT_ROOT/_scripts/.tmp/logs"
# 完整镜像名称
...
RUN apk add --no-cache tzdata bash curl python3 py3-pip
...
# 复制本地构建的前端文件
COPY _scripts/.tmp/static/management.html /CLIProxyAPI/static/management.html
...
DOCKERFILE_EOF
    ...
    if [ "$USE_LOCAL_FRONTEND" = "true" ]; then
        local build_cmd="docker build --network=host -t $FULL_IMAGE_NAME -f _scripts/.tmp/Dockerfile.local --build-arg VERSION=$VERSION --build-arg COMMIT=$COMMIT --build-arg BUILD_DATE=$BUILD_DATE ."
        write_command "$build_cmd" "构建 Docker 镜像（使用本地前端，约 1-5 分钟）"

        if ! docker build --network=host -t "$FULL_IMAGE_NAME" \
            -f _scripts/.tmp/Dockerfile.local \
    ...
    if [ "$USE_SYSTEM_CONFIG" = "true" ]; then
        ...
        if [ -d "$PROJECT_ROOT/$HOOK_SCRIPTS_DIR_NAME" ]; then
            write_filepath "钩子脚本:" "$PROJECT_ROOT/$HOOK_SCRIPTS_DIR_NAME" "(挂载到容器)"
        fi
...
EOF
    ...
    # Hook scripts mount: project's hook-scripts/ -> container's unified-routing data dir
    local hook_scripts_src="$PROJECT_ROOT/$HOOK_SCRIPTS_DIR_NAME"
    local hook_scripts_dst="/root/.cli-proxy-api/unified-routing/hook-scripts"
    local hook_mount_flag=""
    if [ -d "$hook_scripts_src" ]; then
        hook_mount_flag="-v \"${hook_scripts_src}:${hook_scripts_dst}\""
    fi

    # sing-box mount: host binary -> container (for ss:// proxy support in hook scripts)
    local singbox_mount_flag=""
    local singbox_path
    singbox_path=$(which sing-box 2>/dev/null || echo "")
    if [ -n "$singbox_path" ] && [ -f "$singbox_path" ]; then
        singbox_mount_flag="-v \"${singbox_path}:/usr/local/bin/sing-box:ro\""
    fi

    local run_cmd="docker run -d --name $TEST_CONTAINER_NAME --network=host -v \"${config_file_to_mount}:/CLIProxyAPI/config.yaml\" -v \"${auth_dir_to_mount}:/root/.cli-proxy-api\" -v \"${log_dir_to_mount}:/CLIProxyAPI/logs\" ${hook_mount_flag} ${singbox_mount_flag} $FULL_IMAGE_NAME"
    write_command "$run_cmd" "启动测试容器（host 网络，端口 $container_port，$config_desc，$frontend_desc）"

    if [ -d "$hook_scripts_src" ]; then
        echo -e "  ${GRAY}钩子脚本挂载: ${YELLOW}${hook_scripts_src}${GRAY} -> ${hook_scripts_dst}${NC}"
    fi
    if [ -n "$singbox_path" ] && [ -f "$singbox_path" ]; then
        echo -e "  ${GRAY}sing-box 挂载: ${YELLOW}${singbox_path}${GRAY} -> /usr/local/bin/sing-box (ro)${NC}"
    fi

    local extra_mounts=()
    if [ -d "$hook_scripts_src" ]; then
        extra_mounts+=(-v "${hook_scripts_src}:${hook_scripts_dst}")
    fi
    if [ -n "$singbox_path" ] && [ -f "$singbox_path" ]; then
        extra_mounts+=(-v "${singbox_path}:/usr/local/bin/sing-box:ro")
    fi
